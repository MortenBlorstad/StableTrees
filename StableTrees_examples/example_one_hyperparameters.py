...
models = {  
            ...
            "NU": NaiveUpdate(),
            "TR1":TreeReevaluation(delta=0.05, alpha=0.0),
            "TR2":TreeReevaluation(delta=0.05, alpha=0.05),
            "TR3":TreeReevaluation(delta=0.1, alpha=0.05),
            "SL1":StabilityRegularization(),
            "SL2":StabilityRegularization(),
            "SL3":StabilityRegularization(),
            "ABU":AbuTree(),
            "BABU1": BABUTree(min_samples_leaf=5, adaptive_complexity=True,bumping_iterations=1),
            "BABU2": BABUTree(min_samples_leaf=5, adaptive_complexity=True,bumping_iterations=3),
            "BABU3": BABUTree(min_samples_leaf=5, adaptive_complexity=True,bumping_iterations=5)
            }
...
for ds,target, feature in zip(datasets,targets, features):
    ...
    for train_index, test_index in kf.split(X):
        ...
        models = {  
                ...
                "TR1":TreeReevaluation(criterion = criterion,min_samples_leaf=5, adaptive_complexity=True,delta=0.05, alpha=0.0),
                "TR2":TreeReevaluation(criterion = criterion,min_samples_leaf=5, adaptive_complexity=True,delta=0.05, alpha=0.05),
                "TR3":TreeReevaluation(criterion = criterion,min_samples_leaf=5, adaptive_complexity=True,delta=0.1, alpha=0.05),
                ...
                "SL3":StabilityRegularization(criterion = criterion,min_samples_leaf=5, adaptive_complexity=True,gamma=0.75),
    ...
    for name in models.keys():
        ...
        stability_SE_norm = np.std(standard_stability[name]/S2_scale)/np.sqrt(len(mse[name]))
       
        print(f"test - mse: {loss_score:.3f} ({loss_SE:.2f}), stability: {stability_score:.3f} ({stability_SE:.2f})")
        print(f"test - mse: {loss_score/mse_scale:.2f} ({loss_SE_norm:.2f}), stability: {stability_score/S2_scale:.2f} ({stability_SE_norm:.2f})")
        print("="*80)

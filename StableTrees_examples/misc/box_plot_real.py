...
for ds,target, feature in zip(datasets,targets, features):
    ...
    for name in models.keys():
        ...
        standard_stability_all[name] += [score/S_scale for score in standard_stability[name]]
    print()
    print(mse_scale)
    print(mse_all["baseline"])

from matplotlib.lines import Line2D
from matplotlib.ticker import FormatStrFormatter
# create figure and axes
fig, axes = plt.subplots(nrows=1, ncols=1, figsize=(8.27, 11), sharey=True,dpi=500)
...
plt.ylabel("models")
plt.savefig(f"StableTrees_examples\plots\\real_data_boxplot_stab.png")
plt.tight_layout()
plt.close()
...
plt.ylabel("models")
plt.yticks(range(1, len(labels) + 1), labels,fontsize=16)
plt.savefig(f"StableTrees_examples\plots\\real_data_boxplot_mse.png")
plt.tight_layout()
plt.close()
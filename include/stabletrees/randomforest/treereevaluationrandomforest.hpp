...
using namespace std;
class RandomForestTR{
    public:
        explicit RandomForestTR(int _criterion,int n_estimator,int max_depth, double min_split_sample,int min_samples_leaf, bool adaptive_complexity, int max_features,double delta,double alpha);
    ...
    protected:
        ...
        int n_estimator;
        double initial_pred;
        std::vector<TreeReevaluation> forest;
...
RandomForestTR::RandomForestTR(int _criterion,int n_estimator,int max_depth, double min_split_sample,int min_samples_leaf, bool adaptive_complexity, int max_features,double delta,double alpha){
    if(max_depth !=NULL){
...
void RandomForestTR::learn(const dMatrix X, const dVector y, const dVector weights){
...
void RandomForestTR::update(const dMatrix X,const dVector y, const dVector weights){
    ...
    #pragma omp parallel for
        for (int i = 0; i < n_estimator; i++) {
            ...
            dVector y_b = y(ind);
            dVector weights_b = weights(ind);
            forest[i].update(X_b,y_b);
        }
}

dVector RandomForestTR::predict(const dMatrix X){
...
iMatrix RandomForestTR::sample_indices(int start, int end){
